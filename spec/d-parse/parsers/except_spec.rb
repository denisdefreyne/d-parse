describe DParse::Parsers::Except do
  ...
  example { expect(parser).not_to parse('x').and_fail_at(0).line(0).column(0).with_failure('expected any character (not end of file), not \'x\'') }
  example { expect(parser).not_to parse('xa').and_fail_at(0).line(0).column(0).with_failure('expected any character (not end of file), not \'x\'') }
